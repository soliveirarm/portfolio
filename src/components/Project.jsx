...
export default function Project({ cover, title, desc, pageLink, techs }) {
  return (
    <section
      className="group flex flex-col justify-between items-center gap-2.5 rounded-10 p-4 cursor-pointer hover:bg-accent/15 transition"
      onClick={() => window.open(pageLink, "_blank")}
    >
      <img className="w-full object-contain rounded-10" src={cover} />

      <h2 className="text-xl font-title italic text-center">{title}</h2>
